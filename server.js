...
async function buildWeeklyDigest() {
  ...
  // Keep only gold/forex/macro-relevant headlines
  let relevant = headlines.filter(h => isRelevant(h.title));

  // Fallback: if too few match, top up with general headlines so the digest isn't empty
...
// Runs automatically every weekday at 07:00 UTC (London open).
// Posts the live gold price and day-over-day change — no manual work.

let previousClosePrice = null; // tracks yesterday's price in memory for % change
...
// Schedule: weekdays (Mon-Fri) at 07:00 UTC. Format: minute hour day month weekday
...
// Manual trigger — visit anytime to test/force-send early
