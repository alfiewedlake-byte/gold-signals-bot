...
const { postEntrySignal, postStatsRecap } = require('./telegram');
...
// Sends to any chat ID, not just the channel — used for DM replies and
...
cron.schedule('0 21 * * *', async () => {
  ...
  const result = await postStatsRecap(buildStatsMessage(1, 'DAILY'), 'daily');
...
cron.schedule('0 21 * * 0', async () => {
  ...
  const result = await postStatsRecap(buildStatsMessage(7, 'WEEKLY'), 'weekly');
...
cron.schedule('0 21 1 * *', async () => {
  ...
  const result = await postStatsRecap(buildStatsMessage(30, 'MONTHLY'), 'monthly');
...
const ENGAGEMENT_PROMPTS = [
  "💰 How much are you up today? DM me @LiquidityEdgeXAU_bot with your P&L 👇",
  "📸 Show us your profits from today's signals — DM @LiquidityEdgeXAU_bot with a screenshot 👇",
  "🎯 Who caught today's move? DM @LiquidityEdgeXAU_bot and let us know how it went for you.",
  "🔥 Which signal today was your best one? DM @LiquidityEdgeXAU_bot and let us know.",
  "📊 Quick check-in — up, down, or flat today? DM @LiquidityEdgeXAU_bot, be honest 👇",
];
...
const CHITCHAT_MESSAGES = [
  ...
  "☕ How's everyone's session going so far? DM @LiquidityEdgeXAU_bot if you're in the green 🔥",
  ...
  "💬 What pair are you watching most closely today? DM @LiquidityEdgeXAU_bot and let us know.",
...
// ── INCOMING DMs TO THE BOT ───────────────────────────────────
// Telegram calls this URL whenever someone messages the bot directly.
// Auto-replies to the person, and forwards what they said to the
// owner's own Telegram so nothing gets missed.

const AUTO_REPLIES = [
  "Thanks for sharing! 🔥 Logged.",
  "Nice one — appreciate you sharing that! 💰",
...
app.post('/telegram-incoming/:secret', async (req, res) => {
  if (req.params.secret !== WEBHOOK_SECRET) {
    return res.status(403).send('Forbidden');
  }
  ...
  if (!message || !message.text) {
    return res.status(200).send('OK'); // ignore non-text updates (stickers, etc.)
  }

  const senderChatId = message.chat.id;
  const senderName = message.from.username
    ? `@${message.from.username}`
    : message.from.first_name || 'Someone';
  const text = message.text;

  try {
    // Auto-reply to the sender
    await sendTelegramMessageTo(senderChatId, buildAutoReply());

    // Forward to the owner, if OWNER_CHAT_ID is set
    if (OWNER_CHAT_ID) {
      await sendTelegramMessageTo(
        OWNER_CHAT_ID,
        `📩 <b>${senderName}</b> messaged the bot:\n\n${text}`
      );
    }

    res.status(200).send('OK');
  } catch (err) {
    console.error('Incoming message handling failed:', err.message);
